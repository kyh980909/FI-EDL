"""Extract v4 Tables 1-5 from runs/{cifar10,mnist}/.
...
CONTROLLER_VARIANTS = [
    ...
    ("edl_annealed",        "Annealed ($\\lambda_t{=}\\min(t/T,1)$)"),
...
def render_table_main(runs_cifar: Path, runs_mnist: Path) -> Dict[str, str]:
    ...
    for method, label in METHOD_ORDER_T1_T2_T3:
        ...
        if method == "fi_edl":
            cifar_paths = [p for p in cifar_paths
                           if re.search(r"eval_\d+T\d+_cifar10_[a-z0-9]+/metrics\.jsonl$", p)]
            mnist_paths = [p for p in mnist_paths
                           if re.search(r"eval_\d+T\d+_mnist_[a-z0-9]+/metrics\.jsonl$", p)]
    ...
    for variant, label in CONTROLLER_VARIANTS:
        ...
        if variant == "fi_edl":
            paths = [p for p in glob.glob(str(runs_cifar / "fi_edl" / "seed_*" / "*" / "metrics.jsonl"))
                     if re.search(r"eval_\d+T\d+_cifar10_[a-z0-9]+/metrics\.jsonl$", p)]
        elif variant == "edl_annealed":
            paths = [p for p in glob.glob(str(runs_cifar / "edl_l1" / "seed_*" / "*" / "metrics.jsonl"))
                     if re.search(r"eval_\d+T\d+_cifar10_[a-z0-9_]+/metrics\.jsonl$", p)]
        else:
            paths = [p for p in glob.glob(str(runs_cifar / "fi_edl" / "seed_*" / "*" / "metrics.jsonl"))
...
def main(argv: Iterable[str] | None = None) -> None:
    ...
    parser.add_argument("--runs-cifar", type=Path,
                        default=Path("runs/cifar10"))
    parser.add_argument("--runs-mnist", type=Path,
                        default=Path("runs/mnist"))
    parser.add_argument("--out", type=Path,
                        default=Path("results/v4/tables.txt"))
