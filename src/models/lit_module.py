"""Central Lightning module for FI-EDL.

Wires together the four plugin components (backbone / head / loss / score)
via the registry, validates each against its protocol, and exposes the
training/validation/test steps plus optimizer configuration.

All slash-delimited metric names (`train/loss`, `val/acc`, ...) and capitalised
group names (`Loss/Total`, `Metric/Fisher_Trace`, ...) are part of the
downstream reporting contract — do not rename without updating the table/plot
scripts.
"""
...
class FIEDLLightningModule(pl.LightningModule):
    def __init__(self, cfg) -> None:
        super().__init__()
        # OmegaConf DictConfig is not a registered-safe global for torch.load,
        # so we avoid pickling it into the checkpoint.
    ...
    def forward(self, x: Tensor) -> Dict[str, Tensor]:
        ...
        out = self.head(feat)
        out["uncertainty_score"] = self.score_fn(out["alpha"])
        return out
    ...
    def _common_step(self, batch: Any, stage: str) -> Tensor:
        ...
        correct = pred == y
        nan_t = torch.tensor(float("nan"), device=x.device)
        unc_correct = uncertainty[correct].mean() if correct.any() else nan_t
        unc_wrong = uncertainty[~correct].mean() if (~correct).any() else nan_t
        evidence_sum = (out["alpha"].sum(dim=1) - self.cfg.model.num_classes).mean()

        aux = loss_out["aux"]
        lambda_mean = float(aux.get("lambda_mean", float("nan")))
        lambda_std = float(aux.get("lambda_std", float("nan")))
        lambda_min = float(aux.get("lambda_min", float("nan")))
        lambda_max = float(aux.get("lambda_max", float("nan")))
        info = float(aux.get("info", float("nan")))
        info_std = float(aux.get("info_std", float("nan")))
        fisher_trace = float(aux.get("fisher_trace", float("nan")))
    ...
    def predict_scores(self, batch: Any) -> Tensor:
        x, _ = batch
        return self(x)["uncertainty_score"]
    ...
    def configure_optimizers(self):
        ...
        if name == "adam":
            ...
            optim = torch.optim.SGD(params, lr=lr, momentum=self.cfg.optimizer.momentum, weight_decay=wd)
        else:
