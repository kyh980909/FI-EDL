"""EDL loss with a fixed regularization weight lambda (Sensoy et al., 2018).

    total = fit + lambda * KL(Dir(alpha_hat) || Dir(1))

where alpha_hat masks evidence on the target class, and lambda is linearly
annealed from 0 to `lambda_value` over `anneal_epochs` epochs.
"""
